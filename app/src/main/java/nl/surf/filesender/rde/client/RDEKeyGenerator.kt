...
class RDEKeyGenerator(var enrollmentParams: RDEEnrollmentParameters) {
    ...
    private val piccPublicKey : PublicKey = RDEDocument.decodePublicKey(oid, enrollmentParams.piccPublicKey)
    ...
    fun generateKey() : RDEKey {
        ...
        val decryptionParams = RDEDecryptionParameters(oid, pcdPublicKey.encoded, protectedCommand)
    ...
    private fun deriveEncryptionKey(sharedSecret : ByteArray) : ByteArray {
        ...
        val emulatedResponse = AESAPDUEncoder(ksEnc.encoded, ksMac.encoded).write(enrollmentParams.Fcont)
